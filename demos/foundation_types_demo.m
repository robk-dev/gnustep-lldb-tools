// Foundation core-type showcase for the GNUstep LLDB data formatters.
//
// Every local below is a distinct concrete gnustep-base class, so the
// debugger's Variables view exercises each formatter: strings in all their
// storage forms, boxed and tagged numbers, arrays, dictionaries, sets, data,
// dates, NSNull, NSURL, and a custom object. Break on the last line.
//
// This file is also the source of the LLDB API test for the formatters
// (lldb/test/API/lang/objc-gnustep/data-formatters/), so keep the variable
// names stable.
...
int main(int argc, const char *argv[]) {
  @autoreleasepool {
    ...
    NSNumber *unsignedLongLong = @18446744073709551615ULL;   // NSUnsignedLongLongNumber
    ...
    NSSet *colors = [NSSet setWithObjects:@"red", @"green", @"blue", nil]; // GSSet
    ...
    // Other value types.
    NSData *data = [@"Hello, data!" dataUsingEncoding:NSUTF8StringEncoding]; // NSDataMalloc
    NSDate *epoch = [NSDate dateWithTimeIntervalSinceReferenceDate:0]; // GSSmallDate
    ...
    // Message sends to step into. `step` on the first line lands in
    // gnustep-base's -[GSArray count] (through objc_msgSend, which LLDB's
    // GNUstep runtime plugin steps through rather than into); on the second,
    // in this file's -[Account description]. The debugger's step-through
    // plan asks the runtime for the method's address instead of stepping
    // the dispatch assembly.
